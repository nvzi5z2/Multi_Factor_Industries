...
def calculate_factor(df):
    ...
    # 计算开盘价和 VWAP 的加权平均值 (open * 0.00817205) + (vwap * (1 - 0.00817205))
    df['weighted_open_vwap'] = (df['open'] * 0.00817205) + (df['vwap'] * (1 - 0.00817205))

    # 计算过去 60 天的平均成交量 (adv60)
    df['adv60'] = df['volume'].rolling(window=60).mean()

    # 计算过去 8 天 adv60 的总和
    df['sum_adv60'] = df['adv60'].rolling(window=8).sum()

    # 计算 weighted_open_vwap 和 sum_adv60 的 6 天相关性
    df['correlation_price_volume'] = df['weighted_open_vwap'].rolling(window=6).corr(df['sum_adv60'])

    # 对相关性结果进行排名
    df['rank_correlation'] = df['correlation_price_volume'].rank()

    # 计算开盘价与过去 13 天开盘价最低值的差值
    df['open_min_13'] = df['open'] - df['open'].rolling(window=13).min()

    # 对差值进行排名
    df['rank_open_min_13'] = df['open_min_13'].rank()

    # 计算最终因子 (rank_correlation < rank_open_min_13) * -1
    df['factor'] = (df['rank_correlation'] / df['rank_open_min_13'])* -1
