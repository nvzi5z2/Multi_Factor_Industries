...
def calculate_factor(df,window_1,window_2):
    ...
    df['rolling_mean_window2'] = df['ths_free_turnover_ratio_index'].rolling(window=window_2).mean()
        # 计算因子值
    df['factor'] = df['rolling_mean_window1'] / df['rolling_mean_window2']

    return df

# 计算所有股票的因子
def calculate_all_factors(price_path,window_1=20,window_2=10):
    # 获取所有股票代码
...
def main():
    ...
    # 将结果保存到CSV文件中
    filename='turn_rate_ma_change'+str(20)+'D_'+str(10)+'D'
