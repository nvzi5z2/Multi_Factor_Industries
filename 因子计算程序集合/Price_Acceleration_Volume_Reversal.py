...
# 计算因子的函数
def calculate_factor(df, window_1=10, window_2=5, adv_window=20):
    # 确保数据按照时间排序
    df = df.sort_values('time')

    # ---- 整合 ts_rank、delta 和因子的计算 ---- #

    # 计算时间序列排名（ts_rank）
    def ts_rank(series, window):
        return series.rolling(window).apply(lambda x: rankdata(x)[-1] / len(x))

    # 计算均值成交量（adv20）
    df['adv20'] = df['volume'].rolling(adv_window).mean()

    # 计算 ts_rank(close, 10)
    df['ts_rank_close'] = ts_rank(df['close'], window_1)

    # 计算 delta(delta(close, 1), 1)
    df['delta_close'] = df['close'].diff(1).diff(1)

    # 计算 volume / adv20 比值
    df['volume_adv_ratio'] = df['volume'] / df['adv20']

    # 计算 ts_rank(volume / adv20, 5)
    df['ts_rank_volume_adv'] = ts_rank(df['volume_adv_ratio'], window_2)

    # 计算最终的因子值
    df['factor'] =-1*rankdata(df['delta_close']) *rankdata(df['ts_rank_volume_adv'])

    return df

# 计算所有股票的因子
def calculate_all_factors(price_path, window_1=10, window_2=5, adv_window=20):
    # 获取所有股票代码
    ...
    for code in tqdm(all_data_file):
        try:
            ...
            # 计算每一个股票的因子
            factor = calculate_factor(df, window_1, window_2, adv_window)
...
def main():
    ...
    # 将结果保存到CSV文件中
    filename = 'Price_Acceleration_Volume_Reversal'+'10_5_20D'
