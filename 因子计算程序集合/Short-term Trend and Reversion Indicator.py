...
def calculate_factor(df):
    ...
    # 计算 VWAP 和 volume 的相关性 (4天)
    def rolling_corr(series1, series2, window):
        return series1.rolling(window).corr(series2)

    df['corr_vwap_volume'] = rolling_corr(df['vwap'], df['volume'], window=4)

    # 计算 rank(corr_vwap_volume)
    df['rank_corr_vwap_volume'] = df['corr_vwap_volume'].rank()

    # 计算50日平均成交量 (adv50)
    df['adv50'] = df['volume'].rolling(window=50).mean()

    # 计算 rank(low)
    df['rank_low'] = df['low'].rank()

    # 计算 rank(adv50)
    df['rank_adv50'] = df['adv50'].rank()

    # 计算 rank(low) 和 rank(adv50) 的相关性 (12天)
    df['corr_low_adv'] = rolling_corr(df['rank_low'], df['rank_adv50'], window=12)

    # 计算 rank(corr_low_adv)
    df['rank_corr_low_adv'] = df['corr_low_adv'].rank()

    # 计算最终因子：rank_corr_vwap_volume < rank_corr_low_adv
    df['factor'] = -1*(df['rank_corr_vwap_volume'] - df['rank_corr_low_adv'])
