...
def calculate_factor(df,window):
    ...
    # 计算窗口期内的相关性
    df['correlation_rank_high_volume'] = df['rank_high'].rolling(window).corr(df['rank_volume'])

    # 对相关性进行排名
    df['rank_correlation'] = df['correlation_rank_high_volume'].rank(method='first')

    # 计算排名和的3期滚动和
    df['rank_sum'] = df['rank_correlation'].rolling(window).sum()

    # 计算因子值 Alpha#15
    df['factor'] = -1 * df['rank_sum']
...
def main():
    ...
    # 设置计算因子的窗口大小
    for window in tqdm(range(15,100,10)):
        try:
            #设置计算因子的数据路径
