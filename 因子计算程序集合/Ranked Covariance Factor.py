...
def calculate_factor(df,window):
    ...
    # 计算收盘价的排名
    df['rank_close'] = df['close'].rank(method='first')

    # 计算成交量的排名
    df['rank_volume'] = df['volume'].rank(method='first')

    # 计算窗口期内的协方差
    df['cov_rank_close_volume'] = df[['rank_close', 'rank_volume']].rolling(window=window).cov().unstack().iloc[:, 1]

    # 对协方差进行排名
    df['rank_cov'] = df['cov_rank_close_volume'].rank(method='first')

    # 计算因子值 Alpha#13
    df['factor'] = df['rank_cov']
