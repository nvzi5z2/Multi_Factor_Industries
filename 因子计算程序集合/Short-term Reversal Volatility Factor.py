...
def calculate_factor(df, window=10):
    ...
    # 计算过去10天收盘价的Z-score标准化
    df['close_mean'] = df['close'].rolling(window=window).mean()
    df['close_std'] = df['close'].rolling(window=window).std()
    df['zscore_close'] = (df['close'] - df['close_mean']) / df['close_std']

    # 计算当天收盘价与开盘价的比值
    df['close_open_ratio'] = df['close'] / df['open']

    # 计算收盘价/开盘价的Z-score标准化
    df['close_open_mean'] = df['close_open_ratio'].rolling(window=window).mean()
    df['close_open_std'] = df['close_open_ratio'].rolling(window=window).std()
    df['zscore_close_open'] = (df['close_open_ratio'] - df['close_open_mean']) / df['close_open_std']

    # 计算最终因子值
    df['factor'] = (-1 * df['zscore_close']) * df['zscore_close_open']
