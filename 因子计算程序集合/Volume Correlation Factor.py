...
# 计算因子的函数
def calculate_factor(df, window=16):
    # 确保数据按照时间排序
    ...
    # 直接使用VWAP，不进行行业中性化处理
    df['vwap_signal'] = df['vwap'] * 0.728317 + df['vwap'] * (1 - 0.728317)

    # 计算VWAP与成交量的相关性，窗口为4
    df['correlation'] = df['vwap_signal'].rolling(window=4).corr(df['volume'])

    # 对相关性结果应用线性衰减函数，窗口为16
    df['decay_linear'] = df['correlation'].rolling(window=16).apply(lambda x: (16 - 1) * x[-1] + 1 * x[-2], raw=True)

    df['ts_rank'] = df['decay_linear'].rolling(window=8, min_periods=1).apply(lambda x: x.dropna().rank(pct=True).iloc[-1] if len(x.dropna()) > 0 else np.nan)
    # 计算最终因子 (-1 * ts_rank)
