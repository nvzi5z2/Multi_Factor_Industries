...
def calculate_factor(df):
    ...
    # 计算 delta(vwap, 5) - 调整为5天
    df['delta_vwap'] = df['vwap'].diff(periods=5)

    # 计算 decay_linear(delta(vwap, 5), 3) - 调整为3天
    def decay_linear(series, window):
        weights = np.arange(1, window + 1)
        return series.rolling(window).apply(lambda x: np.dot(x, weights) / weights.sum(), raw=True)

    df['decay_delta_vwap'] = decay_linear(df['delta_vwap'], 3)

    # 计算 rank(decay_delta_vwap)
    df['rank_decay_delta_vwap'] = df['decay_delta_vwap'].rank()

    # 计算 (open * 0.147155) + (low * (1 - 0.147155))
    df['weighted_open_low'] = (df['open'] * 0.147155) + (df['low'] * (1 - 0.147155))

    # 计算 delta(weighted_open_low, 2) - 调整为2天
    df['delta_weighted_open_low'] = df['weighted_open_low'].diff(periods=2)

    # 计算 decay_linear(delta_weighted_open_low, 3) - 调整为3天
    df['decay_delta_weighted_open_low'] = decay_linear(df['delta_weighted_open_low'] * -1, 3)

    # 计算 Ts_Rank(decay_delta_weighted_open_low, 17) - 调整为17天
    def ts_rank(series, window):
        return series.rolling(window).apply(lambda x: pd.Series(x).rank().iloc[-1], raw=False)

    df['ts_rank_decay_weighted_open_low'] = ts_rank(df['decay_delta_weighted_open_low'], 17)

    # 计算最终因子：max(rank_decay_delta_vwap, ts_rank_decay_weighted_open_low) * -1
    df['factor'] = np.maximum(df['rank_decay_delta_vwap'], df['ts_rank_decay_weighted_open_low']) * -1
