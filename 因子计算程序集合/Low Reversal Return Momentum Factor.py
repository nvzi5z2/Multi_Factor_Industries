...
def calculate_factor(df, window=5):
    ...
    # 计算5天前的最低价
    df['low_5_delay'] = df['low_5'].shift(5)

    # 使用复利方式计算240天和20天的累计收益率
    df['returns_240'] = (1 + df['returns']).rolling(window=240).apply(np.prod, raw=True) - 1
    df['returns_20'] = (1 + df['returns']).rolling(window=20).apply(np.prod, raw=True) - 1

    # 计算收益率差异并排序
    df['returns_diff'] = (df['returns_240'] - df['returns_20']) / 220
    df['rank_returns_diff'] = df['returns_diff'].rank(pct=True)

    # 计算成交量的时序排名
    df['rank_volume'] = df['volume'].rolling(window=5).apply(lambda x: x.rank(pct=True).iloc[-1], raw=False)
    # 计算因子值
    df['factor'] = (((-1 * df['low_5']) + df['low_5_delay']) * df['rank_returns_diff']) * df['rank_volume']
