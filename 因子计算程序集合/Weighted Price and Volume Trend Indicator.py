...
def calculate_factor(df):
    ...
    # 计算第一部分：low 和 vwap 的加权平均
    weight = 0.35
    df['weighted_price'] = (df['low'] * weight) + (df['vwap'] * (1 - weight))
    
    # 计算19天窗口期内 weighted_price 和 adv40 的滚动求和
    df['sum_weighted_price'] = df['weighted_price'].rolling(window=19).sum()
    df['sum_adv40'] = df['adv40'].rolling(window=19).sum()

    # 计算相关性 (6天窗口期)
    def rolling_corr(series1, series2, window):
        return series1.rolling(window).corr(series2)

    df['corr_weighted_adv40'] = rolling_corr(df['sum_weighted_price'], df['sum_adv40'], window=6)

    # 计算 rank(corr_weighted_adv40)
    df['rank_corr_weighted_adv40'] = df['corr_weighted_adv40'].rank()

    # 计算第二部分：VWAP 和 volume 的 rank 相关性
    ...
    # 计算 rank(corr_vwap_volume)
    df['rank_corr_vwap_volume'] = df['corr_vwap_volume'].rank()

    df['rank_corr_weighted_adv40'] = (df['rank_corr_weighted_adv40'] - df['rank_corr_weighted_adv40'].min()) / (df['rank_corr_weighted_adv40'].max() - df['rank_corr_weighted_adv40'].min())
    df['rank_corr_vwap_volume'] = (df['rank_corr_vwap_volume'] - df['rank_corr_vwap_volume'].min()) / (df['rank_corr_vwap_volume'].max() - df['rank_corr_vwap_volume'].min())

    # 重新计算因子
    df['factor'] = df['rank_corr_weighted_adv40'] * df['rank_corr_vwap_volume']
