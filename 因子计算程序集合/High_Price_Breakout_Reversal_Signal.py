...
# 计算因子的函数
def calculate_factor(df, window_high=20, delta_window=2):
    
    df = df.sort_values('time')

    # 计算几何平均价格 sqrt(high * low)
    df['geom_mean_price'] = (df['high'] * df['low'])**0.5

    # 计算VWAP
    df['vwap'] = (df['close'] * df['volume']).cumsum() / df['volume'].cumsum()

    # 计算几何平均价格与VWAP的差值
    df['factor'] = df['geom_mean_price'] - df['vwap']


    return df

# 计算所有股票的因子
def calculate_all_factors(price_path, window_high=20, delta_window=2):
    # 获取所有股票代码
    ...
    for code in tqdm(all_data_file):
        try:
            ...
            # 计算每一个股票的因子
            factor = calculate_factor(df, window_high, delta_window)
...
def main():
    ...
    # 将结果保存到CSV文件中
    filename = 'High_Price_Breakout_Reversal_Strength_20D_2D'
