...
def calculate_factor(df,window):
    ...
    # 对low价格进行排名
    df['rank_low'] = df['low'].rank(method='first')
    
    # 计算rank(low)的时间序列排名
    df['ts_rank'] = df['rank_low'].rolling(window=window).apply(lambda x: pd.Series(x).rank().iloc[-1])
...
# 计算所有股票的因子
def calculate_all_factors(price_path,window=25):
    # 获取所有股票代码
...
def main():
    ...
    # 将结果保存到CSV文件中
    filename='low_price_rank'+str(25)+'D'
