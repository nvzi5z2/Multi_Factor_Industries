...
def calculate_factor(df,window_1,window_2):
    ...
    df['delta_log_volume'] = df['log_volume'].diff(window_1)

    # 计算 (close - open) / open
    df['close_open_ratio'] = (df['close'] - df['open']) / df['open']

    # 对delta_log_volume和close_open_ratio进行排名
    df['rank_delta_log_volume'] = df['delta_log_volume'].rank(method='first')
    df['rank_close_open_ratio'] = df['close_open_ratio'].rank(method='first')

    # 计算rank(delta(log(volume), 2))与rank(((close - open) / open))的相关性
    df['correlation'] = df['rank_delta_log_volume'].rolling(window=window_2).corr(df['rank_close_open_ratio'])

    # 计算因子值
    df['factor'] = -1 * df['correlation']

    return df

# 计算所有股票的因子
def calculate_all_factors(price_path,window_1=1,window_2=20):
    # 获取所有股票代码
...
def main():
    ...
    #设置计算因子的数据路径
    price_path=r'D:\量化交易构建\市场数据库\数据库\申万二级行业量价数据'
    # 计算所有股票的因子,并且合并在一个表中
    ...
    # 将结果保存到CSV文件中
    filename='Volume-Price Divergence Factor'+str(1)+'D_'+str(20)+'D'
