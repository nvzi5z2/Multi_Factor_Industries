...
# 计算因子的函数
def calculate_factor(df, window_delay=5, window_sum_short=5, window_sum_long=20, window_corr=2):

    # 确保数据按照时间排序
    df = df.sort_values('time')

    # 计算延迟5天的收盘价
    df['delay_close'] = df['close'].shift(window_delay)

    # 计算延迟5天后的20天平均收盘价
    df['sum_delay_close_20'] = df['delay_close'].rolling(window=window_sum_long).sum() / window_sum_long

    # 对20天平均收盘价进行排名
    df['rank_sum_delay_close_20'] = df['sum_delay_close_20'].rank(pct=True)

    # 计算过去2天收盘价与成交量的相关性
    df['corr_close_volume'] = df['close'].rolling(window=window_corr).corr(df['volume'])

    # 计算5天收盘价总和与20天收盘价总和的相关性
    df['sum_close_5'] = df['close'].rolling(window=window_sum_short).sum()
    df['sum_close_20'] = df['close'].rolling(window=window_sum_long).sum()
    df['corr_sum_close'] = df['sum_close_5'].rolling(window=window_corr).corr(df['sum_close_20'])

    # 对相关性进行排名
    df['rank_corr_sum_close'] = df['corr_sum_close'].rank(pct=True)

    # 计算最终因子值
    df['factor'] = -1 * (df['rank_sum_delay_close_20'] * df['corr_close_volume'] * df['rank_corr_sum_close'])
...
def main():
    ...
    all_factors.loc[:,"standard_factor"]=all_factors.loc[:,"factor"]
