...
def calculate_factor(df,window):
    ...
    # 计算收益率的变化
    df['delta_returns'] = df['returns'].diff(3)

    # 对收益率的变化进行排名
    df['rank_delta_returns'] = df['delta_returns'].rank(method='first')

    # 计算开盘价与成交量的滚动相关性
    df['correlation_open_volume'] = df['open'].rolling(window=window).corr(df['volume'])

    # 计算因子值 Alpha#14
    df['factor'] = (-1 * df['rank_delta_returns']) * df['correlation_open_volume']
...
def main():
    ...
    for window in tqdm(range(21,31,1)):
        try:
            #设置计算因子的数据路径
