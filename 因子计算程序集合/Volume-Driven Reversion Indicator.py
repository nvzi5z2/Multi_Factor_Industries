...
def calculate_factor(df):
    ...
    # 计算30日平均成交量 (adv30)
    df['adv30'] = df['volume'].rolling(window=30).mean()

    # 计算 sum(adv30, 37) - 调整为37天
    df['sum_adv30_37'] = df['adv30'].rolling(window=37).sum()

    # 计算 close 和 sum(adv30, 37) 的相关性 (15天)
    def rolling_corr(series1, series2, window):
        return series1.rolling(window).corr(series2)

    df['corr_close_adv'] = rolling_corr(df['close'], df['sum_adv30_37'], window=15)

    # 计算 rank(corr_close_adv)
    df['rank_corr_close_adv'] = df['corr_close_adv'].rank()

    # 计算 (high * 0.0261661) + (vwap * (1 - 0.0261661))
    df['weighted_high_vwap'] = (df['high'] * 0.0261661) + (df['vwap'] * (1 - 0.0261661))

    # 计算 rank(weighted_high_vwap)
    df['rank_weighted_high_vwap'] = df['weighted_high_vwap'].rank()

    # 计算 rank(volume)
    df['rank_volume'] = df['volume'].rank()

    # 计算 rank(weighted_high_vwap) 和 rank(volume) 的相关性 (11天)
    df['corr_high_vwap_volume'] = rolling_corr(df['rank_weighted_high_vwap'], df['rank_volume'], window=11)

    # 计算 rank(corr_high_vwap_volume)
    df['rank_corr_high_vwap_volume'] = df['corr_high_vwap_volume'].rank()

    # 计算最终因子：rank_corr_close_adv < rank_corr_high_vwap_volume * -1
    df['factor'] = -1*(df['rank_corr_close_adv'] /df['rank_corr_high_vwap_volume'])
