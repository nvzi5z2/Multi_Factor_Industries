...
def calculate_factor(df):
    ...
    # 确保 market_cap 使用 totalCapital 列
    df['market_cap'] = df['totalCapital']

    # 计算10天的收益之和
    df['sum_returns_10'] = df['returns'].rolling(window=10).sum()

    # 计算2天的收益之和
    df['sum_returns_2'] = df['returns'].rolling(window=2).sum()

    # 计算3天的和
    df['sum_sum_returns_3'] = df['sum_returns_2'].rolling(window=3).sum()

    # 计算第一个rank项：10天收益之和 / 3天2天和 的rank
    df['rank_1'] = df['sum_returns_10'] / df['sum_sum_returns_3']
    df['rank_1'] = df['rank_1'].rank()

    # 计算第二个rank项：收益 * 市值 的rank
    df['rank_2'] = (df['returns'] * df['market_cap']).rank()

    # 计算最终因子值
    df['factor'] = -(df['rank_1'] * df['rank_2'])
