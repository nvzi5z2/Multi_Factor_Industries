...
# 计算因子的函数
def calculate_factor(df, window=9):
    
    # 确保数据按照时间排序
    df = df.sort_values('time')

    # 计算价格区间的相对位置
    df['position'] = ((df['close'] - df['low']) - (df['high'] - df['close'])) / (df['close'] - df['low'])

    # 计算9天的变化差分
    df['delta_position'] = df['position'].diff(window)

    # 计算因子：乘以-1
    df['factor'] = -1 * df['delta_position']
