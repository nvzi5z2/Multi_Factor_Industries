...
def calculate_factor(df, window=20):
    ...
    # 计算价格延迟
    df['delay_20'] = df['close'].shift(20)
    df['delay_10'] = df['close'].shift(10)
    df['delay_1'] = df['close'].shift(1)

    # 计算两个时间段的价格变化
    df['change_20_10'] = (df['delay_20'] - df['delay_10']) / 10
    df['change_10_now'] = (df['delay_10'] - df['close']) / 10

    # 计算趋势差异
    df['trend_diff'] = df['change_20_10'] - df['change_10_now']
    
    # 返回趋势差异或动量值，而非固定值
    df['factor'] = df['trend_diff'] + (-1 * (df['close'] - df['delay_1']))
