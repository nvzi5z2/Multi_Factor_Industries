...
def calculate_factor(df,window):
    ...
    # 计算VWAP的10日均值
    df['vwap_mean'] = df['vwap'].rolling(window=window).mean()
    
    # 计算开盘价与10日VWAP均值的差值
    df['open_vwap_diff'] = df['open'] - df['vwap_mean']
    
    # 计算收盘价与VWAP的差值
    df['close_vwap_diff'] = df['close'] - df['vwap']
    
    # 对上述差值进行排名
    df['rank_open_vwap_diff'] = df['open_vwap_diff'].rank(method='first')
    df['rank_close_vwap_diff'] = df['close_vwap_diff'].rank(method='first')
    
    # 计算因子值
    df['factor'] = df['rank_open_vwap_diff'] * (-1 * abs(df['rank_close_vwap_diff']))
