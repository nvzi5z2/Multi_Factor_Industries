...
def calculate_factor(df, window_1):
    ...
    def calculate_slope(x):
        ...
        if len(x) < window_1:
            return np.nan
        # 计算时间窗口内换手率的斜率
        y = x['ths_free_turnover_ratio_index'].values
        x_values = np.arange(len(y))  # 使用时间索引作为自变量
        slope, intercept, r_value, p_value, std_err = linregress(x_values, y)
        return slope

    # 通过滚动窗口计算每个时间段的换手率斜率
    df['factor'] = -1*df['ths_free_turnover_ratio_index'].rolling(window=window_1).apply(lambda x: calculate_slope(df.loc[x.index]))

    return df

# 计算所有股票的因子
def calculate_all_factors(price_path,window_1=25):
    # 获取所有股票代码
...
def main():
    ...
    # 将结果保存到CSV文件中
    filename='turn_rate_slope'+str(5)+'D'
