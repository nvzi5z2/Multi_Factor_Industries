...
def calculate_factor(df):
    ...
    # 计算 VWAP 的 3 天变化 (delta)
    df['vwap_delta'] = df['vwap'].diff(3)

    # 计算 VWAP delta 的 7 天线性衰减加权平均
    df['decay_vwap_delta'] = decay_linear(df['vwap_delta'], 7)

    # 对 decay_vwap_delta 进行排名
    df['rank_decay_vwap_delta'] = df['decay_vwap_delta'].rank()

    # 计算 (low * 0.96633) + (low * (1 - 0.96633))，实际上就是 low，因为系数和为1
    df['low_weighted'] = df['low']

    # 计算 ((low_weighted - vwap) / (open - (high + low) / 2))
    df['price_diff_ratio'] = (df['low_weighted'] - df['vwap']) / (df['open'] - (df[['high', 'low']].mean(axis=1)))

    # 计算 price_diff_ratio 的 11 天线性衰减加权平均
    df['decay_price_diff_ratio'] = decay_linear(df['price_diff_ratio'], 11)

    # 对 decay_price_diff_ratio 进行 6 天的时间序列排名
    df['ts_rank_decay_price_diff_ratio'] = df['decay_price_diff_ratio'].rolling(window=6).apply(lambda x: pd.Series(x).rank().iloc[-1], raw=False)
    # 计算最终因子 (rank_decay_vwap_delta + ts_rank_decay_price_diff_ratio) * -1
    df['factor'] = (df['rank_decay_vwap_delta'] + df['ts_rank_decay_price_diff_ratio']) * -1
