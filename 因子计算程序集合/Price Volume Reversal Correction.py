...
def calculate_factor(df):
    ...
    # 2. 计算 ((rank(open) + rank(low)) - (rank(high) + rank(close)))
    df['price_rank_diff'] = (df['rank_open'] + df['rank_low']) - (df['rank_high'] + df['rank_close'])
    
    # 3. 对 price_rank_diff 进行线性衰减
    decay_window_1 = int(8.06882)  # 窗口期约为 8 天
    df['decay_price_rank_diff'] = df['price_rank_diff'].rolling(window=decay_window_1).apply(
        lambda x: np.dot(x, np.arange(1, len(x)+1)) / np.sum(np.arange(1, len(x)+1)), raw=True
    )
    
    # 4. 对 decay_price_rank_diff 进行横截面排名
    df['rank_decay_price'] = df['decay_price_rank_diff'].rank(pct=True)
    
    # 5. 计算 adv60（60 天平均成交量）
    adv_window = 60
    df['adv60'] = df['volume'].rolling(window=adv_window).mean()
    
    # 6. 对 close 和 adv60 进行时间序列排名
    ts_rank_close_window = int(8.44728)  # 窗口期约为 8 天
    ts_rank_adv_window = int(20.6966)  # 窗口期约为 21 天
    df['ts_rank_close'] = df['close'].rolling(window=ts_rank_close_window).apply(
        lambda x: x.rank().iloc[-1] if len(x) == ts_rank_close_window else np.nan, raw=False
    )
    ...
    # 7. 计算 ts_rank_close 和 ts_rank_adv60 的相关性
    corr_window = int(8.01266)  # 窗口期约为 8 天
    df['corr_close_adv'] = df['ts_rank_close'].rolling(window=corr_window).corr(df['ts_rank_adv60'])
    
    # 8. 对相关性取绝对值并进行线性衰减
    df['decay_corr_close_adv'] = df['corr_close_adv'].abs().rolling(window=int(6.65053)).apply(
        lambda x: np.dot(x, np.arange(1, len(x)+1)) / np.sum(np.arange(1, len(x)+1)), raw=True
    )
    
    # 9. 对 decay_corr_close_adv 进行时间序列排名
    ts_rank_corr_window = int(2.61957)  # 窗口期约为 3 天
    df['ts_rank_decay_corr'] = df['decay_corr_close_adv'].rolling(window=ts_rank_corr_window).apply(
        lambda x: x.rank().iloc[-1] if len(x) == ts_rank_corr_window else np.nan, raw=False
    )
    
    # 10. 取 rank_decay_price 和 ts_rank_decay_corr 的最小值
    df['factor'] = np.minimum(df['rank_decay_price'], df['ts_rank_decay_corr'])
