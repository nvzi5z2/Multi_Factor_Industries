...
# 计算因子的函数
def calculate_factor(df, window=1):
    # 确保数据按照时间排序
    df = df.sort_values('time')

    # 计算收盘价相对于最高价和最低价的相对位置，并乘以成交量
    df['price_pos'] = ((df['close'] - df['low']) - (df['high'] - df['close'])) / (df['high'] - df['low']) * df['volume']

    # 对price_pos进行排名并标准化 (scale + rank)
    df['rank_price_pos'] = df['price_pos'].rank(pct=True)
    df['scaled_price_pos'] = (df['rank_price_pos'] - df['rank_price_pos'].mean()) / df['rank_price_pos'].std()

    # 计算过去window天内收盘价最大值出现的时间位置
    df['argmax_close'] = df['close'].rolling(window).apply(lambda x: x.argmax(), raw=True)

    # 对argmax_close进行排名并标准化 (scale + rank)
    df['rank_argmax_close'] = df['argmax_close'].rank(pct=True)
    df['scaled_argmax_close'] = (df['rank_argmax_close'] - df['rank_argmax_close'].mean()) / df['rank_argmax_close'].std()

    # 计算最终因子 (0 - (1 * ((2 * scaled_price_pos) - scaled_argmax_close)))
    df['factor'] = 0 - (1 * ((2 * df['scaled_price_pos']) - df['scaled_argmax_close']))
