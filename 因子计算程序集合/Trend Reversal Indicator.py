...
def calculate_factor(df,window):
    ...
    """
    计算 Trend Reversal Indicator (TRI) 因子：
    ...
    window_min (int): 计算最小值的滚动窗口大小（默认为5天）。
    window_max (int): 计算最大值的滚动窗口大小（默认为5天）。
    
    返回：
    pd.DataFrame: 包含计算出的 TRI 因子值的 DataFrame。
    """
    # 确保数据按照时间排序
