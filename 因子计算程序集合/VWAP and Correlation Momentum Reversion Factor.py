...
def calculate_factor(df, window=22):
    # 确保数据按照时间排序
    ...
    # 计算过去22天的adv20的总和
    df['adv20_sum_22'] = df['adv20'].rolling(window=22).sum()

    # 计算VWAP与adv20_sum_22的9天相关性
    df['correlation_vwap_adv20_sum'] = df['vwap'].rolling(window=9).corr(df['adv20_sum_22'])

    # 对相关性进行排名
    df['rank_correlation'] = df['correlation_vwap_adv20_sum'].rank()

    # 计算开盘价的排名
    df['rank_open'] = df['open'].rank()

    # 计算(high + low) / 2 的排名
    df['rank_mid_price'] = ((df['high'] + df['low']) / 2).rank()

    # 计算最高价的排名
    df['rank_high'] = df['high'].rank()

    # 计算价格动量的排名比较
    df['price_momentum_rank'] = (df['rank_open'] + df['rank_open']) < (df['rank_mid_price'] + df['rank_high'])

    # 对价格动量排名进行排名
    df['rank_price_momentum'] = df['price_momentum_rank'].rank()

    # 计算最终因子 (rank_correlation < rank_price_momentum) * -1
    df['factor'] = -1*(df['rank_correlation'] - df['rank_price_momentum'])
