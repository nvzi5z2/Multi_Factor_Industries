...
def calculate_factor(df,window):
    ...
    # 计算收盘价变化
    df['delta_close'] = df['close'].diff(window)

    # 计算成交量变化的符号
    df['sign_delta_volume'] = np.sign(df['delta_volume'])

    # 计算因子值Alpha#12
    df['factor'] = df['sign_delta_volume'] * (-1 * df['delta_close'])
...
def main():
    ...
    for window in tqdm(range(11,21,1)):
        try:
            #设置计算因子的数据路径
