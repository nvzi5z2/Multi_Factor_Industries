...
# 计算因子的函数
def calculate_factor(df, window_1=20, window_2=20, window_3=7, window_4=8):
    ...
    # 计算 (volume / adv20) 的时间序列排名
    df['volume_adv_ratio'] = df['volume'] / df['adv20']
    df['ts_rank_volume_adv'] = df['volume_adv_ratio'].rolling(window=window_2).apply(
        lambda x: x.rank().iloc[-1], raw=False
    )

    # 计算过去7天收盘价的变化
    df['delta_close_7'] = df['close'].diff(window_3)

    # 计算 -1 * delta(close, 7) 的时间序列排名
    df['ts_rank_close_delta'] = (-1 * df['delta_close_7']).rolling(window=window_4).apply(
        lambda x: x.rank().iloc[-1], raw=False
    )

    # 计算最终因子值
    df['factor'] = df['ts_rank_volume_adv'] * df['ts_rank_close_delta']
